...
from app.api.users.models.user import User, Organization, UserRole
from app.api.users.schemas.user import UserCreate, OrganizationCreate
from app.api.users.utils.auth_utils import get_password_hash
...
def create_user(db: Session, user: UserCreate):
    ...
    db.commit()
    db.refresh(new_user)
    return new_user   # ✅ return ORM object, not dict
...
def create_organization(db: Session, organization: OrganizationCreate):
    ...
    db.commit()
    db.refresh(new_org)
    return new_org   # ✅ return ORM object
