...
def create_notification(notif: NotificationCreate, db: Session = Depends(get_db), current_user=Depends(get_current_user_or_organization)):
    ...
    return create_response(success=True, message="Notification created successfully", data=NotificationOut.from_orm(db_notif))
...
def get_notifications(is_read: bool = None, db: Session = Depends(get_db), current_user=Depends(get_current_user_or_organization)):
    ...
    return create_response(success=True, message="Notifications retrieved successfully", data=[NotificationOut.from_orm(n) for n in notifs])

@router.patch("/{notif_id}/read", response_model=APIResponse[Optional[NotificationOut]])
def mark_as_read(notif_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user_or_organization)):
    ...
    return create_response(success=True, message="Notification marked as read", data=NotificationOut.from_orm(notif) if notif else None)