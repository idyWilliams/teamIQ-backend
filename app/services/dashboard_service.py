...
# NOTE: Skill-related models were not found, so skill logic is disabled.
# To enable, create the Skill and UserSkill models and uncomment the logic.
...
def compute_task_stats(db: Session, user_id: str):
    ...
    completed = db.query(func.count(Task.id)).filter(Task.owner_id == user_id, Task.status == "done").scalar() or 0
...
def compute_timeseries(db: Session, user_id: str, days: int = 30):
    ...
    rows = (
        ...
        .filter(Task.owner_id == user_id, Task.status == "done", Task.completed_at >= since)
        .group_by("day")
        .order_by("day")
        .all()
    )
...
def compute_project_stats(db: Session, user_id: str):
    ...
    active = db.query(func.count(Project.id)).filter(Project.owner_id == user_id, Project.status == "active").scalar() or 0
...
def compute_and_upsert_dashboard_metrics(db: Session, user_id: str):
    ...
    metrics.overall_score = (task_stats["completion_rate"] + project_stats["overall_progress"]) / 2 if task_stats["tasks_total"] else 0.0
...
def compute_org_metrics(db: Session, org_id: str):
    ...
    task_completion_trend = 0.0 # Placeholder
